...
import requests
from nagios import NagiosResponse
...
def main():
    ...
    health_url = 'https://{0}:{1}/v1/status?&details=true'.format(args.host, args.port)
    headers = { "x-api-key" : args.token}

    try:
        health_req = requests.get(url=health_url, timeout=30, verify=args.verify, headers=headers)
